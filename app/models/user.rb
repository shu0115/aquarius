class User < ActiveRecord::Base
  ...
  def self.get_display_name( user_id )
    user = self.find( :first, :select => "login_id, display_name", :conditions => { :id => user_id } )
    unless user.blank?
  ...
  def self.get_twitter_page( user_id )
    ...
    unless user.blank?
      unless user.twitter_id.blank?
        return "<a href='https://twitter.com/#!/#{user.twitter_id}' target='_blank'>#{self.get_display_name( user.id )}</a>"
      else
        return self.get_display_name( user.id )
      end
    else
      return ""
    end
