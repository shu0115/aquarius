class ProtectController < ApplicationController
  ...
  before_filter :show_key_check, :except => [ :index, :show_key_check, :show_key ]

  #-------#
  # index #
  #-------#
  def index
    @elections = Election.paginate( :conditions => { :mode => "protect" }, :page => params[:page], :per_page => $per_page, :order => "created_at DESC" )
  end
  ...
  #----------------#
  def show_key_check
    election = Election.first( :conditions => { :id => params[:id], :mode => "protect" } )

    # 閲覧キーチェック
    if !election.blank? and !election.show_key.blank? and session["show_key_#{election.id}"] != "OK"
    #if true
      redirect_to :action => "show_key", :id => election.id, :return_action => params[:action] and return        
    end
  end
  ...
  #----------#
  def show_key
    @election = Election.find_by_id_and_mode( params[:id], "protect" )

    # 閲覧キー照合
    if @election.show_key == params[:show_key]
      session["show_key_#{@election.id}"] = "OK"
      redirect_to :action => params[:return_action], :id => @election.id and return
    else
